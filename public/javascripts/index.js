        var o = [],
            ...
            vv = [],
            options, mi = false,
            a = getls("connections_2") == null ? 100 : parseInt(getls("connections_2")),
            mpos = {
                x: "",
                y: ""
            },
            ram = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || function(x) {
        ...
        function init_meh() {
            if (!itd()) {
                btn_start.addEventListener("click", switch_g);
            }
            btn_start.addEventListener("touchstart", function(e) {
                // mi = false;
                mpos.x = e.targetTouches[0].pageX
                mpos.y = e.targetTouches[0].pageY;
                switch_g();
            });
            btn_start.addEventListener("touchend", function(e) {
                mpos.x = e.targetTouches[0].pageX
                mpos.y = e.targetTouches[0].pageY;
            });
            btn_start.addEventListener("touchmove", function(e) {
                e.preventDefault();
                mpos.x = e.targetTouches[0].pageX
                mpos.y = e.targetTouches[0].pageY;
            });
            ...
            btn_start.addEventListener("mouseenter", function(e) {
                mpos.x = e.x;
                mpos.y = e.y;
                mi = true;
            });
            btn_start.addEventListener("mouseout", function() {
                mi = false;
            });
            btn_start.addEventListener("mousemove", function(e) {
                mi = true;
                mpos.x = e.x;
                mpos.y = e.y;
            });
            btn_start.addEventListener("mouseover", function(e) {
                mi = true;
                mpos.x = e.x;
                mpos.y = e.y;
            });
        ...
        function dot() {
            if (options.g)
                for (var i = 0; i < a; i++) {
                    ...
                    // ctx.fillRect(o[i].x, o[i].y, 15, 15); //填充的四个参数(x,y,width,height)
                    // ctx.strokeRect(o[i].x, o[i].y, 15, 15); //线的四个参数(x,y,width,height)
                    // ctx.moveTo(350,50);
                    // ctx.lineTo(300,200);
                    // ctx.lineTo(400,300);
                    // ctx.closePath();
